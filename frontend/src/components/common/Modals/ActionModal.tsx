            ...
            <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-full max-w-lg p-6">
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button
                  ...
                  disabled={isDeleting}
                  type="button"
                  className="px-4 py-2 bg-red-600 border border-transparent rounded-md text-sm font-medium text-white
       hover:bg-red-700 transition-colors disabled:opacity-75"
                >
                 {isDeleting ? (
    <span className="flex items-center">
      <PiSpinnerGap className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" />
      {actionText ? `${actionText}ing...` : "Deleting..."} 
    </span>
  ) : (
    actionText || "Delete"
  )}
